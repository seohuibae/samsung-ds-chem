...
def main(args):
    ...
    for test_idx in test_indices:
        ...
        if args.model in ['simple', 'transformer']:
            ...
            optimizer = build_optimizer(model, 'adam', args.lr, args.weight_decay)
        ...
        if not os.path.exists(f'./graphcl-{args.model}-pretrain'):
            os.mkdir(f'./graphcl-{args.model}-pretrain')
        if not os.path.exists(f'./graphcl-{args.model}-finetune'):
            os.mkdir(f'./graphcl-{args.model}-finetune')
        ...
        if not os.path.exists(f'./graphcl-{args.model}-pretrain/{pretrain_model_path}'):
            os.mkdir(f'./graphcl-{args.model}-pretrain/{pretrain_model_path}')
        if not os.path.exists(f'./graphcl-{args.model}-pretrain/{pretrain_model_path}/{args.seed}'):
            os.mkdir(f'./graphcl-{args.model}-pretrain/{pretrain_model_path}/{args.seed}')
        
        if not os.path.exists(f'./graphcl-{args.model}-finetune/{model_path}'):
            os.mkdir(f'./graphcl-{args.model}-finetune/{model_path}')
        if not os.path.exists(f'./graphcl-{args.model}-finetune/{model_path}/{args.seed}'):
            os.mkdir(f'./graphcl-{args.model}-finetune/{model_path}/{args.seed}')
        ...
        if args.finetune_only:
            ...
            model.load_state_dict(torch.load(f'./graphcl-{args.model}-pretrain/{pretrain_model_path}/{args.seed}/pretrained-{args.epochs}-{test_idx}.pth.tar'))
        else: 
            ...
            for epoch in range(args.epochs):
                ...
                # print(res)
                dict2tsv(res, f'./graphcl-{args.model}-pretrain/{pretrain_model_path}/{args.seed}/logs_pretrain-{test_idx}.txt')
                if (epoch+1)%20==0: 
                    torch.save(model.state_dict(), f'./graphcl-{args.model}-pretrain/{pretrain_model_path}/{args.seed}/pretrained-{epoch+1}-{test_idx}.pth.tar')           
            
            torch.save(model.state_dict(), f'./graphcl-{args.model}-pretrain/{pretrain_model_path}/{args.seed}/pretrained-{args.epochs}-{test_idx}.pth.tar')
        ######
        ...
        if args.model in ['simple', 'transformer']:
            ...
            optimizer = build_optimizer(model, 'adam', args.ft_lr, args.weight_decay)
        ...
        for k in range(total_test_iter):    
            ...
            for epoch in range(args.ft_epochs):
                ...
                res['epoch'] = epoch
                res['loss'] = np.mean(training_loss)
                dict2tsv(res, f'./graphcl-{args.model}-finetune/{model_path}/{args.seed}/logs_finetune-{test_idx}.txt')
        ...
        dict2tsv(res, f'./graphcl-{args.model}-finetune/{model_path}/{args.seed}/logs_test-{test_idx}.txt')
    print(model_path)
